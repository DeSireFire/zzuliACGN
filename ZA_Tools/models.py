...
class Gallerys(models.Model):
    imgMd5 = models.CharField('通过hash字段加密的md5字符串，用于其他表调用', default='', max_length=255, unique=True)
    imgName = models.CharField('图片文件名',max_length=255)
    url = models.URLField('图片地址',default='')
    origin = models.URLField('原图片地址',default='')
    hash = models.CharField('图hash或者key',default='',max_length=255)
